...
`chain params` is TRON only — SR-governed system parameters have no EVM counterpart, so an EVM network fails with `family_mismatch`. `chain prices` answers in the selected network's fee model and returns a different field set per family: energy and bandwidth prices on TRON, gas prices on EVM.
