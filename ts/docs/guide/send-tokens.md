...
```bash
wallet-cli token add --contract T... --network tron:3448148188         # TRC20, by contract
wallet-cli token add --contract 0x... --network eip155:11155111     # ERC20, by contract
wallet-cli token add --asset-id 1000001 --network tron:3448148188      # TRC10, TRON only
```
...
> **Mainnet**: the same commands with `--network tron:728126428`, `--network eip155:1` or `--network eip155:56` move real assets, irreversibly. Triple-check `--to` (a confirmed transfer to a wrong address is gone), and dry-run first.
