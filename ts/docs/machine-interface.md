...
| `legacy_derivation` | A seed account uses a historical TRON path this version no longer produces (`m/44'/195'/<n>'/0/0`). Raised when signing that TRON address or deriving a new account in the same wallet. See [Accounts & HD](concepts/accounts-and-hd.md#accounts-derived-before-4131) for migration |
