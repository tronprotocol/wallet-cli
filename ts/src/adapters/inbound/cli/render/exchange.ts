...
function whole(raw: unknown, decimals: unknown): string {
  return raw === undefined || raw === null ? "" : formatDecimal(fromBaseUnits(String(raw), num(decimals, 0)))
}
...
export const ExchangeFormatters = {
  exchangeShow: ((data) => {
    ...
    const reserves = kv([
      [sideLabel(d.firstTokenId, d.firstTokenLabel), whole(d.firstTokenBalance, d.firstTokenDecimals)],
      [sideLabel(d.secondTokenId, d.secondTokenLabel), whole(d.secondTokenBalance, d.secondTokenDecimals)],
    ] as Pair[], "    ")
