...
import { fail, ok, pending, query, receipt, table } from "./layout.js";
...
export const GasFreeFormatters = {
  ...
  gasFreeTransfer: (value: GasFreeTransferView): string => {
    const marker =
      value.stage === "confirmed" ? ok() : value.stage === "failed" ? fail() : pending();
    const title =
      value.stage === "dry-run"
    ...
    const result = receipt(marker, title, [
      ...
      ["Activation fee", amount(value.activateFee, value.decimals, value.token)],
      ["Authorized max fee", amount(value.authorizedMaxFee, value.decimals, value.token)],
      ["Total", amount(value.totalDeducted, value.decimals, value.token)],
      [
        "Status",
        value.state?.toLowerCase() ?? (value.stage === "dry-run" ? "not submitted" : "accepted"),
      ],
  ...
  gasFreeTrace: (value: GasFreeTraceView): string =>
    query([
      ["Trace ID", value.traceId],
