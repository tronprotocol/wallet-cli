...
export const StakeFormatters = {
  stakeInfo: ((data, ctx) => {
    const d = asObj(data);
    const staked = asObj(d.staked);
    const vp = asObj(d.votingPower);
    const res = asObj(d.resource);
    const energy = asObj(res.energy);
    const bandwidth = asObj(res.bandwidth);
    const unfreeze = asObj(d.unfreeze);
    const unfreezing = Array.isArray(d.unfreezing) ? d.unfreezing.map(asObj) : [];
    const totalStaked =
      BigInt(String(staked.energySun ?? "0")) + BigInt(String(staked.bandwidthSun ?? "0"));
    const lines: Pair[] = [
      ["Label", ctx.accountLabel ?? ""],
      [
        "Staked",
        `${trx(totalStaked.toString())}  (for energy ${trx(staked.energySun)} + for bandwidth ${trx(staked.bandwidthSun)})`,
      ],
      [
        "Voting power",
        `${formatInt(vp.total)} TP  (used ${formatInt(vp.used)} / available ${formatInt(vp.available)})`,
      ],
      ...
      [
        "Unfreezing",
        `${unfreezing.length} pending  (max ${formatInt(unfreeze.max)} at a time, ${formatInt(unfreeze.remaining)} more allowed)`,
      ],
    ];
  ...
  stakeDelegated: ((data, ctx) => {
    ...
    const lockText = (v: unknown) =>
      v ? formatAtWithRelative(v) : out ? "not locked" : "none — reclaimable anytime";
    const table = rows.map((r) => [
      String(out ? r.receiver : r.from),
      String(r.resource),
      trx(r.amountSun),
      lockText(r.lockedUntil),
    ]);
