...
function renderVoteStatus(data: Obj, ctx: TextRenderContext): string {
  ...
  const lines = [
    query([
      ...
      [
        "Voting power",
        `${formatInt(votingPower.total)} TP  (used ${formatInt(votingPower.used)} / available ${formatInt(votingPower.available)})`,
      ],
  ...
  for (const vote of votes) {
    if (vote.rewardRatioPct === 0) {
      lines.push(
        `! ${formatInt(vote.count)} votes on ${String(vote.name ?? vote.witness ?? "")} earn nothing — 0% reward ratio`,
      );
    }
  }
  return lines.join("\n");
