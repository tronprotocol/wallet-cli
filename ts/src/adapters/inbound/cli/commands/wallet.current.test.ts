...
function command(
  ...
  const tronNet = { id: "tron:mainnet", family: "tron", nativeSymbol: "TRX", chainId: "mainnet", capabilities: [] };
  const context = {
...
describe("current --qr", () => {
  it("encodes exactly the selected account's TRON address in text mode", async () => {
    ...
    const result = await fixture.current.run(fixture.context as never, fixture.tronNet as never, { qr: true });
...
function withNetwork(
  ...
  const context = { activeAccount: "wlt_selected", output: "text" as const, warn: vi.fn() };
