...
describe("node rejection classification", () => {
  it("names the three rejections we can be sure of", () => {
    ...
    expect(classifyNodeRejection("Validate InternalTransfer error, token required must greater than expected")?.code)
      .toBe("slippage_exceeded");
    expect(classifyNodeRejection("ExchangeTransactionContract is rejected")?.code)
  ...
  it("sees through java-tron's envelope, which is what actually reaches the wire", () => {
    ...
    expect(classifyNodeRejection("Contract validate error : ExchangeTransactionContract is rejected")?.code)
      .toBe("exchange_trading_disabled");
    expect(classifyNodeRejection("Contract validate error : Not precise enough")?.code)
      .toBe("precision_loss");
  });
  ...
  it("does not match a longer message that merely contains a rule's text", () => {
    ...
    expect(classifyNodeRejection("Not precise enough for the withdrawal, but proceeding")).toBeUndefined();
    expect(classifyNodeRejection("ExchangeTransactionContract is rejected by the local policy engine"))
      .toBeUndefined();
  });
});
