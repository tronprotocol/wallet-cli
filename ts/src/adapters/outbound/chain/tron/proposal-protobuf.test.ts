...
describe("exact ProposalCreateContract protobuf", () => {
  it("is byte-identical to TronWeb for safe integers", () => {
    ...
    expect(tronUtils.transaction.txPbToRawDataHex(proposalCreateTxJsonToPbExact(input)))
      .toBe(tronUtils.transaction.txPbToRawDataHex(tronUtils.transaction.txJsonToPb(input)));
  });

  it("encodes the full Java long without Number rounding", () => {
    const payload = proposalCreatePayloadHex(OWNER_HEX, [{ key: 17, value: "9223372036854775807" }]);
...
describe("exact UpdateEnergyLimitContract protobuf", () => {
  ...
  it("is byte-identical to TronWeb for safe values", () => {
    ...
    expect(tronUtils.transaction.txPbToRawDataHex(updateEnergyLimitTxJsonToPbExact(input)))
      .toBe(tronUtils.transaction.txPbToRawDataHex(tronUtils.transaction.txJsonToPb(input)));
  });
