...
it.each(
  ["hex", "decimal", "both"].flatMap((capability) =>
    [
      ...
      "rate-failure",
      "body-rate-failure",
    ].flatMap((outcome) => ["USDT", "USDD"].map((token) => [capability, outcome, token] as const)),
  ),
)(
  "roundtrip negotiates %s support (%s, %s) without retrying payment",
  async (capability, outcome, token) => {
    const networks =
      capability === "hex"
        ? ["tron:0xcd8690dc"]
        : capability === "decimal"
    ...
    const facilitator: typeof fetch = async (url, init) => {
      ...
      if (path === "/settle" && outcome === "rate-failure")
        return new Response(null, { status: 429 });
      if (path === "/settle" && outcome === "body-rate-failure")
        return Response.json({ success: false, httpStatus: 429 });
      return Response.json(
        path === "/verify"
          ? { isValid: true, payer }
    ...
    if (outcome.endsWith("failure")) {
      await expect(pending).rejects.toMatchObject({
        ...(outcome.includes("rate") ? { code: "provider_rate_limited" } : {}),
