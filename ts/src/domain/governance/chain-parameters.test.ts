...
describe("chain parameter proposal mapping", () => {
  it("accepts names and ids, applies last duplicate, and sorts by protocol id", () => {
    expect(parseChainParameterAssignments([
      "getTransactionFee=12",
      "2=200000",
      "GETTRANSACTIONFEE=15",
    ], current)).toEqual([
  ...
  it("rejects unknown parameters and invalid boolean values before building", () => {
    expect(() => parseChainParameterAssignments(["getMissing=1"], current))
      .toThrowError(expect.objectContaining({ code: "unknown_parameter" }));
    expect(() => parseChainParameterAssignments(["getAllowMultiSign=2"], current))
  ...
  it("accepts the full positive Java long range without precision loss", () => {
    expect(parseChainParameterAssignments(["getTotalEnergyLimit=9223372036854775807"], current))
