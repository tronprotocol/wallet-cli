...
describe("TronContractService governance", () => {
  ...
  it("requires SmartContract.origin_address to equal the selected account", async () => {
    ...
    await expect(service.clearAbi(scope, NET, { address: CONTRACT, permissionId: 0 })).rejects.toMatchObject({ code: "not_contract_deployer" })
  ...
  it("maps the generic adapter absence to contract_not_found", async () => {
    ...
    await expect(service.clearAbi(scope, NET, { address: CONTRACT, permissionId: 0 })).rejects.toMatchObject({ code: "contract_not_found" })
  })
