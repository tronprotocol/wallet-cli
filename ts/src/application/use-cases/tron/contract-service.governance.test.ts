...
function createService(gateway: Partial<TronGateway>) {
  ...
  return new TronContractService(
...
describe("TronContractService governance", () => {
  it("applies v4.11 permission and expiration controls to contract send", async () => {
    ...
    await expect(service.send(scope, NET, {
      contract: CONTRACT,
      method: "set(uint256)",
  ...
  it("requires SmartContract.origin_address to equal the selected account", async () => {
    ...
    await expect(service.clearAbi(scope, NET, { address: CONTRACT, permissionId: 0 }))
  ...
  it("passes the caller-paid percentage through without reversing it", async () => {
    ...
    await expect(service.setUserResourcePercent(scope, NET, {
      address: CONTRACT, percent: 100, permissionId: 2,
  ...
  it("accepts an energy limit above TronWeb's obsolete 10M client cap", async () => {
    ...
    await expect(service.setOriginEnergyLimit(scope, NET, {
      address: CONTRACT, energy: 50_000_000, permissionId: 0,
