...
function capture(output: "text" | "json", quiet = false, verbose = false) {
  const out: string[] = [];
  const err: string[] = [];
  const sm = new StreamManager(output, quiet, verbose, (s) => out.push(s), (s) => err.push(s));
  return { sm, out, err };
}
...
describe("StreamManager", () => {
  ...
  it("quiet suppresses diagnostics; debug needs verbose", () => {
    const q = capture("text", true);
    q.sm.diagnostic("info", "x");
    expect(q.err).toEqual([]);

    const v = capture("text", false, false);
    v.sm.diagnostic("debug", "d");
    expect(v.err).toEqual([]);

    const vv = capture("text", false, true);
