...
- **文法 B**:綁鏈命令 `wallet-cli <family> <resource> <action> --network <net> [flags]`(`family` ∈ `tron`/`evm`);中立命令 `wallet-cli wallet|config|chains|capabilities [action]`(無 `--network`,`capabilities` 例外)。
- **flag 標記**(命令表的 flags 欄):`*` = 必填、`[x]` = 選填、`a|b` = 互斥擇一。`--account` 等全域旗標不重列於每命令,定義見 §4 flag 表。
- **需求三碼**(`net · wallet · auth`,值:`req` 必須 / `opt` 視旗標而定 / `—` 不需)——三者各問一件事:
  ...
  - **wallet** = 要不要**選定一個錢包/帳戶**?`—` 不碰錢包(`wallet list`、`chains list`);`opt` 預設用 active、可用 `--account`/`--address` 覆寫(多數讀類查詢);`req` 必須有目標帳戶(簽名、`wallet backup`)。
  - **auth** = 要不要用 **master password 解鎖秘密**?`—` 純讀不解鎖;`req` 要解鎖才能簽名/讀出秘密/加密新檔(`tx send-*`、`wallet backup`、`wallet create`)。
  - 兩者獨立:`account balance` = `wallet opt · auth —`(選帳戶但不解鎖);`wallet create` = `wallet — · auth req`(不選既有帳戶但要密碼加密)。`auth=req` 的簽名命令走 `TxPipeline`,預設 `--dry-run`、顯式 `--broadcast` 才上鏈(= 二次確認);詳見 §6。
- **一期排除**(二期/不做):多簽、delegate/undelegate、投票 SR/領獎、提案、TRC10 發行、Bancor/market、GasFree、NFT、USD 行情估值、DApp 瀏覽器。
...
│  ├─ account   balance | resources | assets | info | history
│  ├─ token     balance | info
│  ├─ tx        send-native | send-token | broadcast | status | info
│  ├─ resource  freeze | unfreeze | withdraw | cancel-unfreeze | prices
│  ├─ block     get
...
| --- | --- | --- | --- |
| `wallet create` | 產生新 HD 錢包(助記詞)並加密存檔 | — · req | `[--label]` `[--words 12\|24]` + `--password-file`(`-stdin` 別名) |
| `wallet import-mnemonic` | 匯入既有助記詞(BIP39)並加密存檔 | — · req | `[--label]` + `--mnemonic-file` + `--password-file`(兩秘密各一條 fd,見 §6) |
| `wallet import-private-key` | 匯入既有私鑰並加密存檔 | — · req | `[--label]` + `--private-key-file` + `--password-file`(兩秘密各一條 fd,見 §6) |
| `wallet import-ledger` | 登記 Ledger(watch-only,簽名走硬體) | — · — | `--app*`、`--index\|--path\|--address`(三擇一)、`[--scan-limit]`、`[--label]` |
| `wallet import-watch` | 登記觀察地址(無秘密) | — · — | `--address*`、`[--label]` |
| `wallet list` | 列出所有錢包/帳戶與 active 標記 | — · — | (無) |
| `wallet set-active` | 設定 active 帳戶 | — · — | `--account*` |
| `wallet active` | 顯示目前 active 帳戶(與 set-active 成對) | — · — | (無) |
| `wallet rename` | 改帳戶/錢包顯示名 | — · — | `--account*`、`--label*` |
| `wallet add-account` | seed 衍生下一個子帳戶(僅 seed) | — · — | `--account*`(指要衍生的 seed 錢包)、`[--label]` |
| `wallet export-address` | 輸出帳戶地址(收款用) | — · — | `[--account]`、`[--family tron\|evm]` |
| `wallet delete` | 刪除錢包/帳戶並清 labels 孤兒 | — · — | `--account*`(帳戶或錢包) |
| `wallet backup` | 匯出帳戶秘密;未帶 `--reveal-secret` 只回 metadata | — · req | `--account*`、`[--reveal-secret]` + `--password-file`(`-stdin` 別名) |
| `config get` | 讀使用者設定 | — · — | `[--key]` |
| `config set` | 寫使用者設定 | — · — | `--key*`、`--value*` |
| `chains list` | 列出已知網路與別名 | — · — | (無) |
| `capabilities` | 列出某網路支援的能力鍵 | (`--network req`) · — | `--network*` |
...
| --- | --- | --- | --- | --- |
| `tx send-native` | 發送 TRX | req · req | `tx.native.transfer` | `--to*`、`--amount-sun*`、`[--dry-run]`、`[--broadcast]`、`[--sign-only]` |
| `tx send-token` | 發送 TRC20/TRC10 | req · req | `tx.token.transfer` | `--to*`、`--amount*`、`--contract\|--asset-id*`、`[--fee-limit]`、`[--dry-run]`、`[--broadcast]`、`[--sign-only]` |
| `tx broadcast` | 廣播預簽交易(不簽名) | — · — | `tx.broadcast` | `--transaction\|--tx-file*`(`--tx-stdin` 為 `--tx-file -` 別名) |
...
### 3.4 `tron resource`(`freeze`/`unfreeze`/`withdraw`/`cancel-unfreeze` 簽名)

| 命令 | 能力 | wallet · auth | capability | 主要 flags |
| --- | --- | --- | --- | --- |
| `resource freeze` | 質押換能量/帶寬(FreezeBalanceV2) | req · req | `staking.freeze` | `--amount-sun*`、`[--resource energy\|bandwidth]`、`[--dry-run]`、`[--broadcast]` |
| `resource unfreeze` | 解押(UnfreezeBalanceV2) | req · req | `staking.freeze` | `--amount-sun*`、`[--resource energy\|bandwidth]`、`[--dry-run]`、`[--broadcast]` |
| `resource withdraw` | 提領已過等待期的解押 TRX(閉合循環) | req · req | `staking.freeze` | `[--dry-run]`、`[--broadcast]` |
| `resource cancel-unfreeze` | 取消全部待解押、回滾為 frozen | req · req | `staking.freeze` | `[--dry-run]`、`[--broadcast]` |
...
| 命令 | 能力 | wallet · auth | capability | 主要 flags |
| --- | --- | --- | --- | --- |
| `message sign` | TIP-191/V2 訊息簽名 | req · req | `message.sign` | `--message\|--message-file*`(`--message-stdin` 為 `--message-file -` 別名) |

---

## 4. Flag 表(意義 + 用到的命令)

> 分類對齊架構 §7.10。同一 flag 在不同命令語義不同者於描述中註明。秘密類旗標永不進 argv(走 stdin / 檔案 / `/dev/fd/N`,`--*-file` 通用形式,`--*-stdin` 為 `--*-file -` 別名),每來源讀一次;多秘密命令各走一條 fd(見 §6 / 架構 §7.13.1)。

| Flag | 類別 | 意義(per-command 差異註明) | 用到的命令 |
| --- | --- | --- | --- |
| `--output text\|json` | 全域 | 輸出格式;`json` 走固定 envelope | 所有命令 |
| `--network <id\|alias>` | 全域 | 選網路(canonical 或別名);動鏈命令(`net=req`)必帶,讀類/離線簽名(`net=opt`)選填——省略則用 family 預設(內建 mainnet,config `defaults.network` 可覆寫);family 須與 positional 一致 | 所有 `tron`/`evm` 命令 + `capabilities` |
...
| `--stream` | 全域 | 僅 `--output json`:長流程中間事件改寫 stdout(NDJSON,末行為終局) | 長流程簽名命令 |
...
| `--password-file <path>` | 含秘密 | 餵 master password,解鎖/加密 keystore。`path` ∈ `-`(stdin)/檔案/`/dev/fd/N`;`--password-stdin` = `--password-file -` 別名。**無 env 來源**(不支援 `MASTER_PASSWORD`) | `wallet create/import-mnemonic/import-private-key/backup`、所有 software 簽名命令 |
| `--private-key-file <path>` | 含秘密 | 餵私鑰;`path` 同上(`--private-key-stdin` 為別名)。與 `--password-file` 各走一條 fd(見架構 §7.13.1) | `wallet import-private-key` |
| `--mnemonic-file <path>` | 含秘密 | 餵助記詞;`path` 同上(`--mnemonic-stdin` 為別名)。與 `--password-file` 各走一條 fd | `wallet import-mnemonic` |
| `--tx-file <path>` | 含秘密/資料 | 餵預簽交易 hex(= `--transaction` 的 file/stdin 形式);`path` 同上(`--tx-stdin` 為別名) | `tx broadcast` |
...
| `--index <n>` | 命令 | Ledger 衍生 index(與 `--path`/`--address` 互斥) | `wallet import-ledger` |
...
| `--family <tron\|evm>` | 命令 | 選輸出哪條鏈的地址 | `wallet export-address` |
| `--reveal-secret` | 命令 | 明確輸出助記詞/私鑰(否則只回 metadata) | `wallet backup` |
| `--key <k>` | 命令 | 設定鍵 | `config get`(選填)、`config set` |
| `--value <v>` | 命令 | 設定值 | `config set` |
| `--to <addr>` | 命令 | 收款地址 | `tx send-native/send-token` |
| `--amount-sun <n>` | 命令 | 金額(SUN);轉帳=轉出額、質押=凍結額 | `tx send-native`、`resource freeze/unfreeze` |
| `--amount <n>` | 命令 | 金額(代幣最小單位) | `tx send-token` |
| `--contract <T...>` | 命令 | TRC20 合約地址;查詢/轉帳/合約通用 | `token balance/info`、`tx send-token`、`contract call/send/info` |
| `--asset-id <id>` | 命令 | TRC10 數字 id(與 `--contract` 互斥) | `token balance/info`、`tx send-token` |
| `--fee-limit <sun>` | 命令 | 能量費上限 | `tx send-token`、`contract send/deploy` |
| `--resource <energy\|bandwidth>` | 命令 | 質押資源類型(預設 bandwidth) | `resource freeze/unfreeze` |
| `--dry-run` | 執行模式 | 只回 plan + 真實費用,不簽不廣播 | 所有簽名命令 |
| `--broadcast` | 執行模式 | 簽名並上鏈(高風險二次確認) | 所有簽名命令 |
| `--sign-only` | 執行模式 | 簽名輸出 hex 不廣播(餵 `tx broadcast`) | `tx send-native/send-token`、`contract send` |
...
| `--limit <n>` | 命令 | 歷史筆數(預設 20) | `account history` |
...
> **不為 EVM 做**:`account resources`、`resource *`、TRC10(`--asset-id`)、`account history`(TronGrid 專屬)、`block get`(暫不順帶)。是否本期就鋪 EVM 順帶可獨立排;加命令只動該 module。
...
- **高風險二次確認** = 預設 `--dry-run`(只回 plan+fee)+ 顯式 `--broadcast` 才上鏈;Ledger 另有硬體按鍵。stateless CLI 無「自動鎖定」,每次調用獨立認證。
- **訪問密碼解析**(`SecretResolver` 集中、每來源讀一次):**唯一來源** `--password-file <path>`(`-`=stdin / 檔案 / `/dev/fd/N`,`--password-stdin` 為別名)。**不支援 `MASTER_PASSWORD` env**(避免裸放 env / process table / shell history)。(待評估)OS 安全輸入互動 fallback 留二期。
- **多秘密命令各走一條 fd**(`import-private-key` = 私鑰 + master password、`import-mnemonic` = 助記詞 + master password):stdin 只有一條,故每個秘密用獨立 `--*-file <path>`,以 process substitution 各開一條匿名 pipe。**fd 0(`-`/`-stdin`)每次調用至多一個秘密占用**;第二個秘密走檔案或 `/dev/fd/N`。CLI 一視同仁 open→read,**不關心來源是 stdin/檔案/Keychain/1Password**——「秘密放哪、要不要生物辨識」外包給 secret manager(如 `op` / `security`),CLI 零整合碼。詳見架構 §7.13.1。

  ```bash
  # 兩秘密各一條 fd,皆不落地、不進 argv、master password 不碰 env
...
- 秘密永不進 argv / log / envelope;`wallet backup --reveal-secret` 是唯一把秘密寫 stdout 的命令,故加顯式旗標閘門。
...
**能力鍵**(架構 §7.9 已宣告):`account.balance.native`/`account.balance.token`、`tx.native.transfer`/`tx.token.transfer`、`tx.estimate`/`tx.broadcast`、`message.sign`、`contract.call`/`contract.deploy`、`resources.energy`/`resources.bandwidth`、`staking.freeze`。

**外部依賴**:
- **TronGrid**(`account history`):`NetworkDescriptor` 增 `tronGridUrl?`,缺值回 `indexer_not_configured`。
- **行情 API**(USD 估值):一期不接,延後;屆時獨立薄 service,不污染純 RPC 路徑。

---

## 8. 建議實作順序(窄而完整,逐段可測)

1. **底層補齊**:§7 RpcClient 方法 + 真實 `estimateResources`/`estimateEnergy`。→ 驗:`send-native --dry-run` 回真實費用。
2. **查詢類(純 RPC)**:`account resources`(含可提領/可解押次數)、`account info`、`token balance`、`token info`、`tx status`、`tx info`、`block get`、`resource prices`、`account assets`。→ golden 測試。
3. **錢包管理補完**:`wallet active`、`wallet delete`、`wallet backup`(+ OS 安全輸入評估)。
4. **轉帳擴充**:`tx send-token`(TRC20/10)+ `--sign-only` + `tx broadcast`。→ dry-run / broadcast / sign-only→broadcast 三態測。
5. **資源質押**:`resource freeze`/`unfreeze`/`withdraw`/`cancel-unfreeze`。→ freeze→unfreeze→withdraw 循環測。
6. **合約底座**:`contract call`(讀先行)/`send`/`deploy`/`info`。
7. **交易歷史**:`account history`(接 TronGrid,`tronGridUrl` 設定 + 缺值錯誤)。
8. (可選)EVM 順帶命令(§5)。
