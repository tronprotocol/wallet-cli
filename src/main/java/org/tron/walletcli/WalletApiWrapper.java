...
//            System.out.println("ovk " + ByteArray.toHexString(expandedSpendingKeyMessage.get().getOvk().toByteArray()));
  ...
  private Pair<GrpcClient, NetType> getGrpcClientAndNetType(String netWorkSymbol, String fulNode,
    String solidityNode) {
    GrpcClient client;
    NetType currentNet;
    if (StringUtils.isEmpty(netWorkSymbol) &&
        (StringUtils.isNotEmpty(fulNode) || StringUtils.isNotEmpty(solidityNode))) {
      client = new GrpcClient(fulNode, solidityNode);
      currentNet = CUSTOM;
    } else {
