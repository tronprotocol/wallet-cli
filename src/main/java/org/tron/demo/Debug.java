...
public class Debug {
  ...
  public static void debug(String privateKey, String hexTransaction)
    ...
    Transaction transaction1 = Transaction.parseFrom(ByteArray.fromHexString(hexTransaction));
    byte[] rawdata = transaction1.getRawData().toByteArray();
    byte[] hash = Sha256Hash.hash(rawdata);
    ECKey ecKey = ECKey.fromPrivate(privateBytes);
    byte[] sign = ecKey.sign(hash).toByteArray();
    transaction1.toBuilder().addSignature(ByteString.copyFrom(sign)).build();
