  ...
  var d = {}, u = d.lib = {}, r = function () {
      ...
      t = u.BufferedBlockAlgorithm = f.extend({
        reset: function () {
          ...
          var b = this._data, c = b.words, j = b.sigBytes, n = this.blockSize,
              e = j / (4 * n),
...
(function (v) {
  ...
  p.WordArray = u.extend({
    init: function (f, s) {
      ...
      for (var f = u.clone.call(this), d = f.words = this.words.slice(0),
          p = d.length, r = 0; r < p; r++) {
        d[r] = d[r].clone();
      }
      return f
    }
...
(function (v) {
  for (var p = CryptoJS, d = p.lib, u = d.WordArray, r = d.Hasher,
      f = p.x64.Word, d = p.algo, s = [], x = [], y = [], e = 1, q = 0, t = 0;
      24 > t; t++) {
  ...
  d = d.SHA3 = r.extend({
    cfg: r.cfg.extend({outputLength: 256}), _doReset: function () {
      ...
      for (var e = this._state, f = this.blockSize / 2, h = 0; h < f; h++) {
        var l = a[b + 2 * h], m = a[b + 2 * h + 1],
            l = (l << 8 | l >>> 24) & 16711935 | (l << 24 | l >>> 8)
                & 4278255360,
            m = (m << 8 | m >>> 24) & 16711935 | (m << 24 | m >>> 8)
                & 4278255360, g = e[h];
        g.high ^= m;
        g.low ^= l
      }
      for (f = 0; 24 > f; f++) {
        ...
        for (m = 1; 25 > m; m++) {
          g = e[m], h = g.high, g = g.low, k = s[m], 32
          > k ? (l = h << k | g >>> 32 - k, d = g << k | h >>> 32 - k) : (l = g
              << k - 32 | h >>> 64 - k, d = h << k - 32 | g >>> 64
              - k), g = c[x[m]], g.high = l, g.low = d;
        }
        g = c[0];
        h = e[0];
        g.high = h.high;
        ...
        for (h = 0; 5 > h; h++) {
          for (k = 0; 5 > k; k++) {
            m = h + 5
                * k, g = e[m], l = c[m], m = c[(h + 1) % 5 + 5 * k], d = c[(h
                + 2)
            % 5 + 5 * k], g.high = l.high ^ ~m.high & d.high, g.low = l.low
                ^ ~m.low
                & d.low;
          }
        }
      ...
      this._process();
      for (var a = this._state, b = this.cfg.outputLength / 8, c = b / 8,
          e = [], h = 0; h < c; h++) {
        var d = a[h], f = d.high, d = d.low,
            f = (f << 8 | f >>> 24) & 16711935 | (f << 24 | f >>> 8)
                & 4278255360,
            d = (d << 8 | d >>> 24) & 16711935 | (d << 24 | d >>> 8)
                & 4278255360;
        e.push(d);
        e.push(f)
      }
      ...
      for (var a = r.clone.call(this), b = a._state = this._state.slice(0),
          c = 0; 25 > c; c++) {
        b[c] = b[c].clone();
      }
      return a
